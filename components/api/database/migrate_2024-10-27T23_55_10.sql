...
GRANT USAGE ON SCHEMA events_public TO event_emitter;
CREATE OR REPLACE PROCEDURE events_public.notify_entity_deleted(entity_id TEXT, entity_type TEXT)
    LANGUAGE sql AS
$$
SELECT
    pg_notify('dansdata_events_v1'::TEXT, JSON_BUILD_OBJECT('type', 'delete', 'entity',
                                                            JSON_BUILD_OBJECT('id', entity_id, 'type', entity_type))::TEXT);
$$;

COMMENT ON PROCEDURE events_public.notify_entity_deleted(entity_id TEXT, entity_type TEXT) IS 'Emit a notification that the specified entity was deleted.';
...
CREATE TABLE IF NOT EXISTS translations_private.ext_refs (
    ...
    CONSTRAINT external_refs_external_id_external_type_uniq
        UNIQUE (external_id, external_type)
);

COMMENT ON TABLE translations_private.ext_refs IS 'References to externally owned objects.';

COMMENT ON COLUMN translations_private.ext_refs.id IS 'Internal identifier for this reference to an external object.';

COMMENT ON COLUMN translations_private.ext_refs.external_id IS 'Externally owned identifier. Must be unique for a given (external_id, external_type) pair.';

COMMENT ON COLUMN translations_private.ext_refs.external_type IS 'Type identifier of the external object.';

ALTER TABLE translations_private.ext_refs
    OWNER TO translations_owner;
CREATE OR REPLACE FUNCTION translations_private.ensure_ext_ref(
    external_id TEXT,
    external_type TEXT
) RETURNS INTEGER
    LANGUAGE sql AS
$$
WITH
    allocate AS ( INSERT INTO translations_private.ext_refs (external_id, external_type) VALUES
                                                                                             (
                                                                                                 ensure_ext_ref.external_id,
                                                                                                 ensure_ext_ref.external_type
...
SELECT
    ...
    WHERE
        translations_private.ext_refs.external_id = ensure_ext_ref.external_id AND
        translations_private.ext_refs.external_type = ensure_ext_ref.external_type;
$$;

COMMENT ON FUNCTION translations_private.ensure_ext_ref(external_id TEXT, external_type TEXT) IS 'Retrieves the internal id to use for the given external entity, generating one if it does not already exist.';

ALTER FUNCTION translations_private.ensure_ext_ref(external_id TEXT, external_type TEXT) OWNER TO translations_owner;
...
COMMENT ON COLUMN translations_private.metadatas.ext_id IS 'Id that can be used by external entities to refer to this translation.';
...
$$
BEGIN
    CALL events_public.notify_entity_deleted(old.ext_id::TEXT, 'translation'::TEXT);
    RETURN NULL;
END;
$$;
...
CREATE TABLE IF NOT EXISTS translations_private.languages (
    ...
    code TEXT NOT NULL
        CONSTRAINT languages_code_uniq UNIQUE,
...
CREATE OR REPLACE PROCEDURE translations_public.drop_ext_ref(
    owner_id TEXT,
    owner_type TEXT
)
    SECURITY DEFINER
...
DELETE
    FROM
        translations_private.ext_refs
    WHERE
        external_id = owner_id AND
        external_type = owner_type;
$$;

COMMENT ON PROCEDURE translations_public.drop_ext_ref(owner_id TEXT, owner_type TEXT) IS 'Delete any references to an external entity and release all associated resources.';

ALTER PROCEDURE translations_public.drop_ext_ref(owner_id TEXT, owner_type TEXT) OWNER TO translations_owner;
...
CREATE OR REPLACE FUNCTION translations_public.allocate_translation(owner_id TEXT, owner_type TEXT) RETURNS uuid
    SECURITY DEFINER
    LANGUAGE sql AS
$$
...
SELECT
    translations_private.ensure_ext_ref(owner_id, owner_type)
    RETURNING ext_id;
$$;

COMMENT ON FUNCTION translations_public.allocate_translation(owner_id TEXT, owner_type TEXT) IS 'Allocates a new translation id for use by the given owner.';

ALTER FUNCTION translations_public.allocate_translation(owner_id TEXT, owner_type TEXT) OWNER TO translations_owner;
...
SELECT
    ...
    WHERE
        ext_id = set_translation.translation_id
...
INSERT INTO translations_private.ext_refs (id, external_id, external_type) OVERRIDING SYSTEM VALUE VALUES (1, 'sv', 'language');
INSERT INTO translations_private.ext_refs (id, external_id, external_type) OVERRIDING SYSTEM VALUE VALUES (2, 'en', 'language');
INSERT INTO translations_private.ext_refs (id, external_id, external_type) OVERRIDING SYSTEM VALUE VALUES (3, 'no', 'language');
